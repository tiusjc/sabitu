...
class form_cadastro_model extends CI_Model {
  ...
  if( $requerido ){
    ...
    if( $grid && $add_edit ){
      $query = $this->db->query("SELECT group_concat( IF(type='UM-PARA-MUITOS',label,field) ORDER BY ordem ) AS field FROM $tabela_campos WHERE form_id= $id AND grid = $grid AND add_edit=$add_edit");
    }else{
      if( $grid){
          $query = $this->db->query("SELECT group_concat( IF(type='UM-PARA-MUITOS',label,field) ORDER BY ordem ) AS field FROM $tabela_campos WHERE form_id= $id AND grid = $grid");
       }else{
          $query = $this->db->query("SELECT group_concat( IF(type='UM-PARA-MUITOS',label,field) ORDER BY ordem ) AS field FROM $tabela_campos WHERE form_id= $id AND add_edit = $add_edit");
       }
    }
  }
